...
这里介绍 NumPy 中与 `np.std()` 类似的各种科学计算函数，涵盖统计、数学、线性代数等多个领域。
...
```python
# 差分
diff_arr = np.diff([1, 2, 4, 7, 11])  # [1, 2, 3, 4]

# 梯度计算
gradient = np.gradient(arr_2d)  # 数值梯度

# 插值
from scipy import interpolate
x = np.array([0, 1, 2, 3, 4])
y = np.array([0, 1, 4, 9, 16])
f = interpolate.interp1d(x, y, kind='cubic')  # 三次样条插值

# 卷积
conv_result = np.convolve([1, 2, 3], [4, 5, 6], mode='full')  # 卷积运算

# 快速傅里叶变换（FFT）
from scipy.fft import fft, ifft
signal = np.array([1, 2, 3, 4])
freq_domain = fft(signal)  # 频域表示
time_domain = ifft(freq_domain)  # 逆变换回时域
```
...
# 检查数据类型
print(big_array.dtype)  # float64（8字节/元素）

# 转换为float32节省内存
small_array = big_array.astype(np.float32)  # 4字节/元素，内存减半

# 使用视图而非副本
view_array = big_array[:100, :100]  # 视图，不复制数据
copy_array = big_array[:100, :100].copy()  # 创建副本
```
...
这个指南涵盖了NumPy中最常用的科学计算函数。实际使用中，建议根据具体需求选择合适的方法，并注意函数的参数和返回值类型。对于更高级的功能，可以查阅SciPy、Pandas等库的文档。